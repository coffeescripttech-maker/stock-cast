    ...
    <div className="animate-[fadeUp_0.25s_ease] space-y-6 max-w-[1600px] mx-auto">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 p-6">
        ...
        <p className="text-xs text-slate-400 dark:text-slate-500 mb-4">
          Latest sales with sale type breakdown
        </p>
      ...
      <div className="space-y-3">
        ...
        {items.slice(0, 5).map((p: any) => {
          ...
          const reorderQty = Math.ceil(
            threshold * 5 - stock + (isWS ? 50 : 30)
          );
          const days =
            stock > 0
              ? `~${Math.max(1, Math.round(stock / 3))} day${Math.round(stock / 3) > 1 ? 's' : ''}`
              : 'No sales data';
          const aiMsg = isWS
            ? stock < 5
              ? 'Critically insufficient. Emergency bulk restock required.'
              : stock < 12
                ? 'Cannot fulfill standard wholesale orders. Consider supplier negotiation.'
                : 'Stock below wholesale buffer. Reorder to fulfil bulk customer demand.'
            : stock < 3
              ? 'Critically low for retail. Schedule reorder within 24–48 hrs.'
              : 'Approaching retail minimum. Reorder to maintain shelf availability.';
          ...
          return (
            <div
              ...
              <div className="text-xs text-slate-400 dark:text-slate-500 mb-3">
                ₱{isWS ? p.wholesalePrice : p.retailPrice}{' '}
                {isWS ? 'WS' : 'retail'} · Category: {p.category || 'Others'}
              </div>
