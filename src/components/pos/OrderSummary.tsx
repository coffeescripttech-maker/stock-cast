...
import { Heart, UserX, Sparkles, Loader2, AlertTriangle, ShoppingBag, QrCode } from 'lucide-react';
...
export function OrderSummary({ onCheckout, onClear, onOpenNFC, submitting }: OrderSummaryProps) {
    ...
    <div className="space-y-4">
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-brand to-brand-dark text-white p-6 shadow-lg">
        ...
        <div className="relative space-y-1">
          <div className="text-xs font-semibold text-white/70 uppercase tracking-wider">Order Total</div>
          <div className="text-3xl font-black font-mono tracking-tight">₱{fmtCurrency(grandTotal)}</div>
        </div>

        <div className="relative mt-4 pt-4 border-t border-white/20 space-y-1.5">
          <div className="flex justify-between text-xs text-white/80">
            <span>{totalUnits} unit{totalUnits !== 1 && 's'} · {cart.length} line{cart.length !== 1 && 's'}</span>
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div className="text-left">
                <div className="text-sm font-bold text-slate-700 dark:text-slate-200">Link Customer</div>
                <div className="text-[11px] text-slate-400">Tap NFC card to earn loyalty points</div>
              </div>
            </div>
      ...
      <div className="space-y-2.5">
        ...
        >
          ...
          {isReady && (
            <kbd className="font-mono text-[10px] px-2 py-0.5 rounded bg-white/20">⌘F8</kbd>
          )}
        </button>
      ...
      <Dialog open={clearConfirmOpen} onOpenChange={setClearConfirmOpen} title="">
        <div className="text-center py-4">
          ...
          <h3 className="text-lg font-bold text-slate-900 dark:text-slate-100 mb-1">Clear Cart?</h3>
          <p className="text-sm text-slate-500">
            This will remove all {cart.length} item{cart.length !== 1 && 's'} from the cart and unlink the customer.
          </p>
          <div className="flex justify-center gap-3 mt-6">
            <Button variant="secondary" onClick={() => setClearConfirmOpen(false)}>Cancel</Button>
            <Button variant="danger" onClick={handleConfirmClear}>Clear Cart</Button>
          </div>
        </div>
      </Dialog>
    </div>
    ...
    <div className="divide-y divide-slate-100 dark:divide-slate-700/50">
      ...
      <div className="flex items-center justify-between p-4">
        <div className="flex items-center gap-3">
          <div className={cn(
            ...
            tier.key === 'gold' ? 'bg-amber-400 ring-amber-200 dark:ring-amber-800' :
            tier.key === 'silver' ? 'bg-slate-400 ring-slate-200 dark:ring-slate-600' :
      ...
      <div className="px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-mono text-slate-500">{customer.points} pts</span>
        </div>
        ...
        {canRedeem ? (
          ...
          >
            {redeemPoints > 0 ? (
              <>
                <Sparkles size={14} />
                <span>Redeeming −₱{Math.floor(redeemPoints / (rewardsConfig.redeemEvery || 100)) * (rewardsConfig.redeemValue || 10)}</span>
              </>
            ) : (
              <>
                <Heart size={14} />
